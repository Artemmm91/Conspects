...
`std::list<int, Allocator<int>> lst;` Вот создали лист. Но как аллоцировать для `Node` а не для `int`? В аллокаторе была внутренняя структура `rebind` с помощью которой можно было создавать другие типы. Но в C++20 это удалили, а также удалили construct и destroy, потому что они везде одинаковые.   
